...
function FilmForm(props) {
    ...
    const handleSubmit = (event) => {
        ...
        if (film) {
            handleUpdateFilm(new Film(film.id, title, favorite, watchDate, rating, film.user));
        } else {
            const newFilm = { title: title, favorite: favorite, watchDate: dayjs(watchDate), score: rating };
    ...
    const [favorite, setFavorite] = useState(film ? film.isFavourite == 1 : false);
    const [watchDate, setWatchDate] = useState(film ? film.watchDate.split("T")[0] : "");
