...
const checkFilm = (req, res, next) => {
    ...
    if (!Number.isInteger(film.rating) || film.rating < 1 || film.rating > 5) {
        error["errors"].push("Invalid rating. It must be an integer between 1 and 5.");
    }
    if (!dayjs(film.watchDate, 'YYYY-MM-DD', true).isValid()) {
    ...
    const allowedFields = ['title', 'isFavorite', 'rating', 'watchDate', 'userId'];
